...
export async function paginate<T>(
  model: {
    ...
    find: (filter: Record<string, unknown>) => {
      skip: (n: number) => {
        limit: (n: number) => {
          sort: (sort: Record<string, unknown>) => Promise<T[]>;
        };
      };
    };
  },
...
export class CustomerService {
  ...
  static async update(
    id: string,
    updateData: Partial<CustomerType>
...
export class AnalyticsService {
  static async getDashboardAnalytics(): Promise<{
    ...
    return {
      ...
      recentOrders,
      topProducts,
      userGrowth: userGrowth.map((item: { _id: string; count: number }) => ({
        date: item._id,
        count: item.count,
      })),
    };
  ...
  > {
    ...
    return revenueData.map((item: { _id: string; revenue: number }) => ({
      month: item._id,
